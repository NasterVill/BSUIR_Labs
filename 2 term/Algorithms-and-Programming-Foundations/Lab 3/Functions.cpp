...
void median_filtration(IMAGE_24* image_st)		// размер аперутуры в данной функции 3х3
{
	...
	for (int i = 0; i < image_st->i_header.biHeight; i++)
	{
		for (int j = 0; j < image_st->i_header.biWidth; j++)
		{
			...
			if (i == image_st->i_header.biHeight - 1 || i == 0) // если р€д крайний
			{
				row_limit = 2;
				if(i == 0) row = i;
			}
			...
			for (int x = row, k = 0; row_limit--; x++)
			{
				...
				{
					red_array[k] = image_st->raster[x][y].rgbRed;		// заполн€ем апертру
					green_array[k] = image_st->raster[x][y].rgbGreen;	// дл€ каждого цвета
					blue_array[k] = image_st->raster[x][y].rgbBlue;		// по отдельности

					k++;		// индекс одномерных массивов, хран€щих цвета пикселей
					size = k;
				}
			}
...
void securedNumerInput(char* input_message, TYPE data...) // первым аргументом в списке пиременных параметров €вл€етс€
{														  // нижн€€ граница ввода, после нее идет верхн€€, а затем указатель
